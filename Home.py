...
from rag_system import get_medical_chain
...
st.title('🩺Dr. Navi')
st.markdown("의료 상담 챗봇입니다. 증상을 입력하면 진료과 추천, 주의사항 등을 안내해드립니다.")
st.markdown("팀원 : 김민철, 임동성, 지용석, 최유정, 허현준")

# message 기록 저장 초기화 -> messages
if "messages" not in st.session_state:
  st.session_state["messages"] = []
  save_message("안녕하세요! 의료 상담 챗봇입니다. 증상을 입력하면 진료과 추천, 주의사항 등을 안내해드립니다.\n\nex) 콧물과 기침이 나와요", "ai")
...
if message: # 사용자가 입력한 메시지가 있으면
    ...
    with st.spinner("Thinking..."):
      ...
      medical_chain = get_medical_chain(llm, vectorstore)
    
    # ai가 대답한 메시지를 그립니다.
    with st.chat_message("ai"):
      medical_chain.invoke(message)
