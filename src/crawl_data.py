...
def crawl_disease_symptom():
    ...
    url = "https://www.amc.seoul.kr/asan/healthinfo/disease/diseaseList.do?diseaseKindId=C00000{}".format(
        20)
    ...
    for link in soup.select('div.tabSearchList.cont2')[0].find_all('a'):
        ...
        link_url = "https://www.amc.seoul.kr/asan/healthinfo/disease/diseaseList.do?diseaseKindId=C{}".format(
            link_code)
        ...
        for page in range(1, total_pages + 1):
            # 페이지가 하나이면 원래 link
            page_url = f"{link_url}&pageIndex={page}" if total_pages > 1 else link_url
            response = requests.get(page_url)
            page_soup = BeautifulSoup(response.content, 'html.parser')
            disease_elements = page_soup.select(
                'ul.descBox > li > div.contBox')  # 질병 정보

            for disease_element in disease_elements:
                disease_name = disease_element.find(
                    'strong').get_text(strip=True)  # 질병명
